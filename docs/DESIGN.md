...
| F4 | 外观调节 | 透明度滑杆(0-100%)、背景设置(纯色/渐变/图片)、字体颜色、主题包 |
...
- 背景图支持本地图片;Electron `vibrancy`(macOS)/`backgroundMaterial`(Win11)做系统级毛玻璃,不支持的平台降级半透明;
...
独立常规窗口(非悬浮窗),从托盘或卡片右键菜单打开。页面:

1. **厂商管理**:内置厂商列表(启用/停用、LOGO 预览)+ "添加自定义厂商";
2. **厂商编辑器**(schema 驱动表单):基本信息 → 鉴权(风格/字段)→ 请求(URL/方法/超时)→ 解析路径。**测试请求按钮**:真实发一次请求,把响应 JSON 渲染成可点击树,点选字段即自动填入解析路径;测试通过才允许保存;
3. **账户管理**:每厂商的账户增删改(Key 输入框防窥视),火山类展示 AK/SK/region 三字段;
4. **显示**:透明度滑杆、尺寸、背景、主题选择、字体颜色覆盖、轮询间隔、自动翻页;
