...
### 9.2 内容布局(两种展示形式,设置页可切换)

- **轮播(默认)**:单卡片逐家切换(自动翻页间隔可配 / 左右箭头);**只在已配置的厂商间轮播**;全部未配置时显示一张示例卡片(演示数据 + "配置任意厂商后展示真实用量"提示);
- **列表**:全部已配置厂商紧凑列表一屏展示(每行:LOGO / 名称 / 主窗口迷你进度条 / 百分比或余额),窗口高度按内容自适应;
- 多账户:同厂商多账户在轮播中各占一页,列表中同名厂商带账户别名;
